            ...
            try:
                ...
                if raw_response:
                    ...
                    if '"should_trigger": false' in raw_response or '"should_trigger":false' in raw_response:
                        print(f"👁️ [Chat] {model_used} 分析完成，但认为无需回复")
                        # 通过信号将结果发回主线程
                        self.vision_thread.vision_reaction.emit('###JSON###{"expression": "smile", "motion": "Idle"}\n嗯...没什么特别的。')
                    else:
                        print(f"👁️ [Chat] {model_used} 分析完成，触发回复")
                        # ✅ 通过信号将结果发回主线程进行UI渲染和语音播放
    ...
    def on_vision_reaction(self, raw_text: str):
        ...
        try:
            ...
            if clean_full_text:
                ...
                # 非语音模式下，才在气泡显示时触发动作
                if (not VISION_USE_VOICE) and pending_motion:
