...
请提取以下四类信息（如果无明显信息则留空数组）：

1. [Facts] 明确的事实更新
   - 姓名、年龄、职业、居住地、人际关系等。
   - 格式: {"key": "name", "value": "张三"}

2. [Psychology] 用户的心理状态、深层需求或性格特征
   - 例如："用户在压力大时倾向于反讽"

3. [Behavior] 用户的日常行为习惯、时间规律或偏好活动 (★新增)
   - ⚠️ 仅记录表现出【重复性】或【显著特征】的行为。
   - ❌ 拒绝记录：
     • 一次性的偶然行为（如"用户今天19:29在工作" → 太具体，不记）
     • 显而易见的系统交互（如"用户使用了视觉功能" → 系统日志，不记）
     • 过于宽泛的描述（如"用户在晚上工作" → 无意义，不记）
   - ✅ 推荐记录：
     • 总结性的规律（如"用户经常在周三/周五晚间处理工作事务"）
     • 时间-活动关联（如"用户习惯在深夜23点左右看B站视频"）
     • 偏好与习惯（如"用户喜欢在写代码时听纯音乐"）
   - 格式: 描述性短句字符串

4. [Summary] 这段对话的主题摘要（一句话概括）

⚠️ 输出格式必须为严格的 JSON:
{
    "facts": [{"key": "occupation", "value": "程序员"}],
    "psychology": ["用户最近工作压力较大"],
    "behavior": ["用户习惯在深夜23点左右看视频放松", "周五下午通常会摸鱼"],
    "summary": "用户深夜还在工作，AI进行了吐槽和陪伴。" 
}
"""
